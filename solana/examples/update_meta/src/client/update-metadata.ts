import {
  ...
  const so_path = path.join(program_path, 'update_auth.so');

  const keypair_path = path.join(program_path, 'update_auth-keypair.json');
...
export async function update_data(): Promise<void> {
    console.log("updating data", greetedPubkey.toBase58());
...
export async function check_data(): Promise<void> {
